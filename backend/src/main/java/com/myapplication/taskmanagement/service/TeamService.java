        ...
        return teamMembers.stream()
                    ...
                    .map(team -> {
                        List<TeamMember> activeMembers = teamMemberRepository
                                .findAllByTeamIdAndActive(team.getId(), true);
                        TeamMember me = activeMembers.stream()
                                .filter(m -> m.getUser().getId().equals(userId))
                                .findFirst().orElse(null);
                        TeamResponse res = teamMapper.toTeamResponse(team);
                        res.setMemberCount(activeMembers.size());
                        res.setMyRole(me != null ? me.getRole().name() : null); //
                        res.setTeamMembers(activeMembers.stream()
